...
export default function Contact() {
  ...
  return (
    <>
      <Header />
      <main className="pt-32 pb-20 px-4 sm:px-6 min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-700">
        <div className="container mx-auto max-w-4xl">
          <h1 className="text-4xl font-bold text-center mb-12 sm:mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 px-4 sm:px-0">
            {contacts.map((contact) => (
              <div 
                ...
                className={`${contact.color} backdrop-blur-sm border border-white/10 rounded-xl p-6 cursor-pointer transition-all duration-300 hover:scale-[1.02] hover:shadow-lg group flex-1 min-h-[180px] flex flex-col justify-between`}
              >
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                    <h3 className="text-lg font-semibold text-white/90 mb-2">{contact.name}</h3>
                    <p className="text-white/80 break-words">{contact.value}</p>
                  </div>
                </div>
                {(contact.id === 1 || contact.id === 2) && (
                  <div className="mt-4 text-right">
                    <button 
                      ...
                      className="text-sm px-3 py-1 rounded-md bg-white/10 hover:bg-white/20 text-cyan-400 hover:text-cyan-300 transition"
                    >
          ...
          <div className="mt-16 text-center text-white/60 px-4">
            <p className="text-lg mb-2">N'hésitez pas à me contacter via l'un de ces canaux</p>
            <p className="text-sm">Cliquez sur une carte pour accéder directement</p>
          </div>
        </div>
      </main>
      <Footer />
